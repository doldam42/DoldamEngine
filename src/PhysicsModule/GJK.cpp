...
struct point_t;
float EPA_Expand(const Collider *pA, const Collider *pB, const float bias, const point_t simplexPoints[4],
...
struct point_t
{
    ...
    Vector3 ptA; // The point on pA
    Vector3 ptB; // The point on pB
...
point_t Support(const Collider *pA, const Collider *pB, Vector3 dir, const float bias)
{
    ...
    point.ptA = pA->pShape->Support(dir, pA->Position, pA->Rotation, bias);

    // Find the point in B furthest in the opposite direction
    point.ptB = pB->pShape->Support(-dir, pB->Position, pB->Rotation, bias);
...
bool GJK_DoesIntersect(const Collider *pA, const Collider *pB, const float bias, Vector3 *ptOnA, Vector3 *ptOnB)
{
    ...
    int     numPts = 1;
    point_t simplexPoints[4];
    simplexPoints[0] = Support(pA, pB, Vector3(1, 1, 1), 0.0f);
    ...
    if (1 == numPts)
    {
        ...
        point_t newPt = Support(pA, pB, searchDir, 0.0f);
        simplexPoints[numPts] = newPt;
        numPts++;
    }
    if (2 == numPts)
    ...
    //
    EPA_Expand(pA, pB, bias, simplexPoints, ptOnA, ptOnB);
    return true;
}
                 ...
                 Vector3 *ptOnA, Vector3 *ptOnB)
{
    ...
    while (1)
    {
        ...
        const point_t newPt = Support(pA, pB, normal, bias);
