...
enum TEXTURE_FILE_FORMAT { TEXTURE_FILE_FORMAT_M };
...
public:
  ...
  HRESULT CreateVertexBuffer(ID3D12Resource          **ppOutBuffer,
                             ...
                             UINT64 sizePerVertex, UINT numVertex,
                             const void *pInitData);
  HRESULT CreateIndexBuffer(ID3D12Resource         **ppOutBuffer,
                            D3D12_INDEX_BUFFER_VIEW *pOutIndexBufferView,
                            UINT numIndices, const void *pInitData);
  HRESULT CreateConstantBuffer(ID3D12Resource **ppOutBuffer, UINT64 sizePerCB,
                               const void *pInitData);
  void    UpdateTextureForWrite(ID3D12Resource *pDestTexResource,
                                ID3D12Resource *pSrcTexResource);

  HRESULT CreateTextureFromMemory(ID3D12Resource **ppOutResource, UINT width,
                                  UINT height, DXGI_FORMAT format,
                                  const BYTE *pInitImage);

  BOOL CreateTextureFromDDS(ID3D12Resource     **ppOutResource,
                               D3D12_RESOURCE_DESC *pOutDesc,
                               const wchar_t       *filename);
  BOOL CreateTextureFromWIC(ID3D12Resource     **ppOutResource,
                               D3D12_RESOURCE_DESC *pOutDesc,
                               const wchar_t       *filename);

  HRESULT CreateDDSTextureFromFile(ID3D12Resource **ppOutResource,
                                   const wchar_t   *filename,
