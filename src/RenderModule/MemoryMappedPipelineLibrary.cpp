...
static constexpr void ThrowIfFailed(HRESULT hr)
{
#ifdef _DEBUG
    if (FAILED(hr))
        __debugbreak();
...
bool MemoryMappedPipelineLibrary::Init(ID3D12Device *pDevice, std::wstring filename)
{
    ...
    if (pDevice)
    {
        ...
        if (SUCCEEDED(pDevice->QueryInterface(IID_PPV_ARGS(&device1))))
        {
            ...
            switch (hr)
            {
            case DXGI_ERROR_UNSUPPORTED: // The driver doesn't support Pipeline libraries. WDDM2.1 drivers must support
                                         // it.
                break;
            ...
            case D3D12_ERROR_DRIVER_VERSION_MISMATCH: // The provided Library contains data from an old driver or
                ...
                MemoryMappedFile::Init(filename);
                if (FAILED(device1->CreatePipelineLibrary(GetData(), GetSize(), IID_PPV_ARGS(&m_pipelineLibrary))))
                {
                    __debugbreak();
                }
                break;
    ...
    if (!deleteFile && m_pipelineLibrary)
    {
        ...
        if (librarySize > 0)
        {
            ...
            if (neededSize > m_currentFileSize)
            {
                ...
                if (pTempData)
                {
                    if (FAILED(m_pipelineLibrary->Serialize(pTempData, librarySize)))
                    {
                        __debugbreak();
                    }
