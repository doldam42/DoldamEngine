...
IRenderMesh *PrimitiveGenerator::MakeSquare(const float scale)
{
    ...
    pObj->BeginCreateMesh(pVertices, 4, nullptr, 0, 1);
...
IRenderMesh *PrimitiveGenerator::MakeSquareGrid(const int numSlices, const int numStacks, const float scale)
{
    ...
    IRenderMesh *pObj = g_pRenderer->CreateMeshObject();
    pObj->BeginCreateMesh(vertices.data(), vertices.size(), nullptr, 0, 1);
...
IRenderMesh *PrimitiveGenerator::MakeBox(const float scale)
{
    ...
    IRenderMesh *pObj = g_pRenderer->CreateMeshObject();
    pObj->BeginCreateMesh(pVertices, 24, nullptr, 0, 1);
...
IRenderMesh *PrimitiveGenerator::MakeWireBox(const Vector3 center, const Vector3 extends)
{
    vector<Vector3> positions;
    vector<Vector3> normals;
    positions.reserve(24);
...
//
// MeshData PrimitiveGenerator::MakeCylinder(const float bottomRadius,
...
IRenderMesh *PrimitiveGenerator::MakeSphere(const float radius, const int numslices, const int numstacks)
{
...
// MeshData PrimitiveGenerator::MakeIcosahedron() {
//
//  // Luna DX12 교재 참고
//  // 등20면체
...
//}
//
// MeshData PrimitiveGenerator::MakeTetrahedron() {
//
//  // Regular Tetrahedron
...
//  return meshData;
//}
// MeshData PrimitiveGenerator::SubdivideToSphere(const float radius,
//                                              MeshData    meshData) {
//
