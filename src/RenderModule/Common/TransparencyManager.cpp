...
void TransparencyManager::SetRootDescriptorTable(UINT ThreadIndex, ID3D12GraphicsCommandList *pCommandList)
{
    ...
    pCommandList->SetGraphicsRoot32BitConstant(3, m_maxFragmentListNodeCount, 0);
...
void TransparencyManager::ResolveOIT(UINT threadIndex, ID3D12GraphicsCommandList *pCommandList,
                                     D3D12_VIEWPORT *pViewport, D3D12_RECT *pScissorRect,
                                     D3D12_CPU_DESCRIPTOR_HANDLE renderTarget)
{
    ...
    pCommandList->SetGraphicsRoot32BitConstant(0, m_maxFragmentListNodeCount, 0);
...
void TransparencyManager::EndRender()
{
    ...
    if (m_allocatedNodeCount >= m_maxFragmentListNodeCount)
    {
        while (m_allocatedNodeCount >= m_maxFragmentListNodeCount)
        {
            m_maxFragmentListNodeCount *= 2;
        }

        UINT maxFragmentListbufferSizeInByte = m_maxFragmentListNodeCount * sizeof(FragmentListNode); // 192MB

        m_pFragmentListRecycleBin->Free(m_pFragmentList, MAX_PENDING_FRAME_COUNT);
        m_pFragmentList = m_pFragmentListRecycleBin->Alloc(maxFragmentListbufferSizeInByte);

        CD3DX12_CPU_DESCRIPTOR_HANDLE cpuHandle(m_descriptorTable.cpuHandle, m_srvDescriptorSize,
        ...
        m_pD3DDevice->CreateUnorderedAccessView(m_pFragmentList, m_pFragmentList, &uavDesc, cpuHandle);
        cpuHandle.Offset(m_srvDescriptorSize, 2);
        ...
        srvDesc.Buffer.StructureByteStride = sizeof(FragmentListNode);
        srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        m_pD3DDevice->CreateShaderResourceView(m_pFragmentList, &srvDesc, cpuHandle);
    }
