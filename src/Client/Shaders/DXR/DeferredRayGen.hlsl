...
void RayGen() {
    ...
    float4      diffuse = diffuseTex.Sample(linearWrapSampler, texcoord);
    float3      albedo = diffuse.xyz;
    const float opacity = diffuse.w;

    float4 N = normalTex.Sample(pointClampSampler, texcoord);
    float3 normalWorld = N.xyz;
    float3 emission = N.w;

    if (IsBlack(normalWorld))
    {
        gOutput[launchIndex] = float4(albedo, 1.0);
        return;
    }

    float4 elements = elementsTex.Sample(pointClampSampler, texcoord);
    const float ao = elements.r;
    const float roughness = elements.g;
    const float metallic = elements.b;
    const float Kr = elements.a;

    float depth = depthTex.Sample(pointClampSampler, texcoord).r;
