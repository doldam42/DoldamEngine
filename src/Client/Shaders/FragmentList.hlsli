...
Texture2D<uint>   gFragmentListFirstNodeAddressSRV : register(t18);

RWStructuredBuffer<FragmentListNode> gFragmentListNodesUAV : register(u1);
StructuredBuffer<FragmentListNode>   gFragmentListNodesSRV : register(t19);
...
float4 FL_UnpackColor(uint packedInput)
{
    ...
    uint4  p = uint4((packedInput & 0xFFUL), (packedInput >> 8UL) & 0xFFUL, (packedInput >> 16UL) & 0xFFUL,
                     (packedInput >> 24UL));
...
uint FL_GetFirstNodeOffset(int2 screenAddress) { return gFragmentListFirstNodeAddressSRV[screenAddress]; }
...
FragmentListNode FL_GetNode(uint nodeAddress) { return gFragmentListNodesSRV[nodeAddress]; }
