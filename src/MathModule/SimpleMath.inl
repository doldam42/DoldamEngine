...
inline float DirectX::SimpleMath::Plane::Distance(const Vector3 &position) const noexcept 
{
    return DotCoordinate(position) + D();
}

inline bool Plane::IsOutside(const Vector3 &position) const noexcept { Distance(position) > 0.0f; }
